...
const LINK_STYLES: Record<string, { color: (t: any) => string; dash: string; width: number; animate?: boolean }> = {
  ethernet: { color: t => t.hi || t.primary, dash: "8 4", width: 2, animate: true },
  wifi:     { color: t => t.acc || t.accent, dash: "2 4", width: 2, animate: false },
  vpn:      { color: t => "#a78bfa", dash: "10 3 3 3", width: 2, animate: true },
  trunk:    { color: t => "#22d3ee", dash: "0", width: 3, animate: false },
  wan:      { color: t => "#fbbf24", dash: "12 6", width: 2.5, animate: true },
  docker:   { color: t => "#0ea5e9", dash: "4 2", width: 1.5, animate: false },
};
...
export function TopologyMap({ theme: t }: Props) {
  ...
  const useCisco = t.id === "enterprise";
  ...
  useEffect(() => {
    setPositions((cur) => {
      const next = { ...cur };
      // Layered fallback when no posX/posY
      const layers: Record<number, any[]> = { 0: [], 1: [], 2: [], 3: [], 4: [] };
      for (const d of devices) {
        let layer = 4;
        if (d.type === "firewall") layer = 1;
        else if (d.type === "router" || d.isMainRouter) layer = 2;
        else if (d.type === "switch" || d.type === "ap") layer = 3;
        layers[layer].push(d);
      }
      const layerY = [200, 380, 580, 880, 1200];
    ...
    return (
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", gap: 12, alignItems: "center", justifyContent: "center", background: t.bg, color: t.muted, fontFamily: t.monoFont, fontSize: 14 }}>
        ...
        <button onClick={() => useStore.getState().triggerScan()}
          style={{ background: t.grad, border: "none", color: "#000", padding: "8px 18px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: t.headFont }}>
          ↻ Trigger scan
        </button>
      </div>
    );
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", width: "100%", height: "100%", background: t.bg, overflow: "hidden", cursor: linkingFrom ? "crosshair" : draggingId ? "grabbing" : panStart ? "grabbing" : "default" }}
      ...
      onMouseMove={onMouseMove} onMouseUp={(e) => onMouseUp(e)} onClick={onCanvasClick} onWheel={onWheel}>
      ...
      <svg ref={svgRef} width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" onContextMenu={(e) => e.preventDefault()}>
        ...
        <defs>
          <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke={t.id === "minimal" ? "#e5e7eb" : "rgba(255,255,255,0.04)"} strokeWidth={0.6}/>
          </pattern>
        </defs>
        ...
        <g ref={worldGroupRef} transform={`translate(${pan.x} ${pan.y}) scale(${zoom}) translate(${(W - W * zoom) / (2 * zoom)} ${(H - H * zoom) / (2 * zoom)})`}>
          ...
          {topology.links.map((l: any) => {
            ...
            const upstreamTypes = ["router", "switch", "firewall", "ap"];
            ...
            return (
              <g key={d.id}
                ...
                <text x={0} y={sz / 2 + 16} textAnchor="middle"
                  fill={t.id === "minimal" ? "#374151" : t.txt}
        ...
        return (
          <div style={{ position: "absolute", left: 16, bottom: 50, background: t.bg, border: `1px solid ${t.border}`, borderRadius: t.radius, padding: 12, minWidth: 240, fontFamily: t.monoFont, fontSize: 11, zIndex: 25, boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
            ...
            {[
              ...
              ["Type", d.customType || d.type], ["Status", d.status],
      ...
      {linkMenu && (() => {
        ...
        if (!l) return null;
        return (
          <div style={{ position: "fixed", left: linkMenu.x, top: linkMenu.y, background: t.bg, border: `1px solid ${t.border}`, borderRadius: 10, padding: 4, zIndex: 9000, minWidth: 200, boxShadow: "0 12px 32px rgba(0,0,0,0.5)" }}>
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "14px 20px", borderTop: `1px solid ${t.border}`, display: "flex", justifyContent: "flex-end", gap: 8 }}>
          ...
          <button onClick={submit} disabled={!name} style={{ background: t.grad, border: "none", color: "#000", padding: "8px 18px", borderRadius: 7, cursor: name ? "pointer" : "not-allowed", fontFamily: t.headFont, fontSize: 12, fontWeight: 700, opacity: name ? 1 : 0.5 }}>{zone.isNew ? "Create zone" : "Save changes"}</button>
        </div>
      </div>
    </div>
  );
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "14px 20px", borderTop: `1px solid ${t.border}`, display: "flex", justifyContent: "flex-end", gap: 8 }}>
          ...
          <button onClick={submit} disabled={busy || !form.customName}
            style={{ background: t.grad, border: "none", color: "#000", padding: "8px 18px", borderRadius: 7, cursor: (busy || !form.customName) ? "not-allowed" : "pointer", fontFamily: t.headFont, fontSize: 12, fontWeight: 700, opacity: (busy || !form.customName) ? 0.5 : 1 }}>
